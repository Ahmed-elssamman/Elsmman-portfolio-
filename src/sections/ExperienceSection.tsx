...
export function ExperienceSection({ data }: { data: Experience }) {
  return (
    <SectionShell
      ...
      index="02"
      eyebrow="Experience"
      title="Where the systems thinking has shipped."
...
function RoleCard({ role, i }: { role: ExperienceItem; i: number }) {
  ...
  return (
    <motion.li
      ...
      transition={{ duration: 0.8, ease: easing.enter, delay: i * 0.05 }}
      className="relative glass-panel hairline rounded-sm p-5 md:p-6"
    >
      <div className="flex flex-col md:flex-row md:items-start gap-4 md:gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
            ...
            <span className="font-mono text-[12px] uppercase tracking-[0.2em] text-accent/90">
              @ {role.company}
            </span>
          </div>
          {role.summary && (
