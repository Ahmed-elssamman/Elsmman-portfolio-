...
export function EcosystemSection({ data }: { data: Ecosystem }) {
  ...
  return (
    <SectionShell
      ...
      index="05"
      eyebrow="Ecosystem"
      title="A galaxy of tools, not a list of skills."
        ...
        <div className="md:col-span-4 space-y-3">
          {data.map((c) => (
            <button
              ...
              className={
                "group w-full text-left glass-panel hairline rounded-sm p-5 transition border " +
                (c.id === activeId
                  ? "border-accent/50"
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{ background: `hsl(${200 + c.hue}, 70%, 65%)` }}
                />
                <span className="font-display text-lg tracking-tight">
                  {c.name}
                </span>
              </div>
              <p className="mt-2 text-ink-mute text-sm">{c.summary}</p>
            </button>
          ))}
        </div>
...
function Galaxy({ cluster }: { cluster: EcosystemCluster }) {
  ...
  return (
    <div className="relative glass-panel hairline rounded-sm overflow-hidden">
      ...
      <div className="relative aspect-[16/11] bg-bg-elev/40">
        <svg
          viewBox="0 0 100 100"
        ...
        >
          {[28, 38, 46].map((r, i) => (
            <ellipse
              ...
              ry={r * 0.6}
              fill="none"
              stroke={`hsla(${baseHue}, 70%, 65%, ${0.12 - i * 0.02})`}
              strokeWidth={0.15}
            />
          ))}
          <circle
        ...
        {nodes.map((n, i) => {
          ...
          return (
            ...
            >
              <div
                className="relative font-mono text-[10px] sm:text-[11px] uppercase tracking-[0.2em] px-2 py-1 sm:px-3 sm:py-1.5 border whitespace-nowrap"
                style={{
                  ...
                  boxShadow: `0 0 ${
                    8 + n.weight * 18
                  }px hsla(${baseHue}, 80%, 60%, ${0.15 + n.weight * 0.18})`,
                }}
              >
                {n.label}
                <span
